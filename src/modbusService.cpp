...
_Error ModbusService::setup(_Application& _theApp) {

    const JsonObject &root = _theApp.getConfig().getJsonObject("modbus");
    if(!root.success()) {
    ...
    _Error err = this->rs485.setup(_theApp.getLogger(), config_rs485);
...
void ModbusService::updateDataMemoryValues() 
{
    //COILS
    {
...
void ModbusService::buildDataMemory() {
