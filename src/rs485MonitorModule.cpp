...
#define TIME_PACKET_END_MS 500  // time to separate monitored packets 



static _Error postToHttpServer(String url, uint8_t * const frameBuffer, const uint16_t frameBuffer_len );
            ...
            switch(this->status) 
            {
                ...
                case 1: //reading packet till the end or timeout
                {
                    while(pSerial->available())
                    {
                        ...
                        {
                            // LOG: end of packet -> too big
                            postToHttpServer(this->monitor_post_rest_url, this->frameBuffer, this->frameBuffer_len);
                    ...
                    if(time_elapsed_sinceLastDataIn > TIME_PACKET_END_MS)
                    {
...
static _Error postToHttpServer( String url, uint8_t * const frameBuffer, const uint16_t frameBuffer_len )
{
    ...
    if(!ok) {
        ...
        if(httpCode < 0) {
            ret  = _Error ( httpCode, String( "HTTP post to url: ")
                + url.c_str() 
                + ", returnCode: <"
                + httpCode
                + ">, error: <"
